...
def execute(masterForm):
    ...
    if (lt == None or str(lt.买卖标志) == "买入"):
        return;
    dao = Cd.Model.非业务车辆费用Dao();#Hd.Model.HdBaseDao[Cd.Model.非业务车辆费用]();
    item = Cd.Model.非业务车辆费用();
    item1 = Cd.Model.非业务车辆费用();
    with Feng.ServiceProvider.GetService[Feng.IRepositoryFactory]().GenerateRepository[Cd.Model.车辆机油]() as rep:
        try:
            ...
            #item.车辆编号 = lt.车辆编号;
            #item.车辆承担 = True;
            if (lt.车主编号 != "900001" and str(lt.买卖标志) == "卖出"):
                item.车辆承担 = False;
                item1.费用实体 = lt;

...
def execute(masterForm):
        def DoWork():
                try:
                        ...
                        cd_boxList = db.ExecuteDataView("select * from 视图查询交互_对账辅助_货代 where 对账单号 = '" + dzd + "'");
                        if (cd_boxList.Count == 0):
                                Feng.Windows.Forms.MessageForm.ShowInfo("没有费用记录！");
                                return -1;
                        cd_dataReader = cd_boxList.Table.CreateDataReader();                       
                        tran = db.BeginTransaction();
                        while (cd_dataReader.Read()):
                                db.ExecuteNonQuery("insert Temp_对账单费用查验(相关人,提单号,箱号,费用项,金额) values ('900008','" + str(cd_dataReader["提单号"]) + "','" \
                        ...
                        #Feng.Windows.Forms.MessageForm.ShowInfo(ex.Message);
                        Feng.ServiceProvider.GetService[Feng.IExceptionProcess]().ProcessWithNotify(ex);
        def WorkDown(result):
                if (result >= 0):
                        Feng.Windows.Forms.MessageForm.ShowInfo("已导入" + str(result) + "条数据！");                        
        
        checkForm = Feng.ServiceProvider.GetService[Feng.IWindowFactory]().CreateWindow(Feng.ADInfoBll.Instance.GetWindowInfo("业务财务_货代对账辅助_货代导入"));
